...
Analysis:
- after cache expires(send the request for multiple times) we observe that the cb query is being reflected in the response --> we can inject a malicious script directly 
	![](Pasted%20image%2020241105215810.png)

- we have to add our cb value now into another header as `Origin:` 
	  ![](Pasted%20image%2020241105221903.png)
  - now we can inject without affecting other users in tests
    ![](Pasted%20image%2020241105222157.png)
 ...
 **Understanding `utm_content` Exclusion from Cache Key**
- The `utm_content` parameter is typically used for tracking marketing campaigns and is not usually needed for caching. Therefore, it’s **excluded from the cache key**. This means that variations in `utm_content` values won’t create new cache entries; all requests with different `utm_content` values will return the same cached response.
- **Key point**: Parameters like `utm_content` are "unkeyed," so their presence or variation doesn’t affect the cache’s stored response.

**Using `utm_content` to Introduce Another Parameter with a Semicolon**
- By appending `utm_content` with a **semicolon (`;`) followed by another parameter**, you can bypass cache key restrictions and add extra parameters. These extra parameters are also excluded from the cache key, so they won’t create new cache entries either.

Analysis:
	![](Pasted%20image%2020241106014612.png)

- adding a second callback parameter where we will inject the payload and we see that the back end server prefers the second call back param
	![](Pasted%20image%2020241106015111.png)

- we need to obfuscate it from the front end cache server by searching an unkeyed param ( in this case we found `utm_content` param)
	![](Pasted%20image%2020241106015413.png)![](Pasted%20image%2020241106015719.png)
- if we modify the utm_content value we will still obtain cache hit
	 if we modify the second callback param we will obtain the cache miss(is cached by the server again) (remember that we want callback param to be part of the utm_content not separated). In this case we need to use another symbol than `&` for param cloaking
	![](Pasted%20image%2020241106020208.png)
# 6. Web cache poisoning via a fat GET request
...
2. Notice that you can control the name of the function that is called in the response by passing in a duplicate `callback` parameter via the request body. Also notice that the cache key is still derived from the original `callback` parameter in the request line:a
